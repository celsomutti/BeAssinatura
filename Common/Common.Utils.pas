...
type
  ...
  public
    ...
    class Function Dialog(sTitulo: String; sTexto: String; iTipo: Integer): Integer;
...
uses Global.Parametros, View.Dialog;
...
begin
  ...
  if FileExt = '.xls' then
    ExportGridToExcel(sFile, grdGrid, False, True, False)
...
class function TUtils.Dialog(sTitulo, sTexto: String; iTipo: Integer): Integer;
begin
  Result := IDCANCEL;
  if not Assigned(view_Dialog) then
  begin
    view_Dialog := Tview_Dialog.Create(Application);
    view_Dialog.Tipo := iTipo;
    view_Dialog.Titulo := stitulo;
    view_Dialog.Mensagem := sTexto;
  end;
  view_Dialog.ShowModal;
  Result := view_Dialog.Resultado;
  FreeAndNil(view_Dialog);
end;
